...
def translate_batch(translator, batch_texts, cache):
    ...
    try:
        ...
        if len(translated_list) != len(batch_texts):
            logging.warning(f"Batch dimension mismatch: Expected {len(batch_texts)}, got {len(translated_list)}. Falling back to individual translation.")
            translated_list = []
        ...
        return translated_list, 0
         ...
         return batch_texts, len(batch_texts) # Return original texts on error so we don't wipe data
...
def translate_cwmp_model(filename="tr-181-2-16-0-cwmp-full.xml"):
    ...
    logging.info(f"Starting API batch translation for {len(nodes_to_translate)} remaining tags.")
    ...
    try:
        ...
        with tqdm(total=len(nodes_to_translate), desc="Processing Batches", unit="tags") as pbar:
            for node, clean_text_str in nodes_to_translate:
                ...
                if current_batch_len + estimated_addition > MAX_BATCH_LENGTH:
                     ...
                     translated_texts, err = translate_batch(translator, current_batch_texts, cache)
                     error_count += err
                     if err == 0:
        ...
        logging.warning("Translation process interrupted by user. Saving cache and exiting.")
    finally:
        save_cache(cache)
        logging.info(f"Translation finished. Fresh API hits: {translated_count}, Cache hits: {cached_count}, Errors: {error_count}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Translate TR-069 CWMP Model descriptions to Japanese (Batched PoC)")
    parser.add_argument("--file", type=str, default="tr-181-2-16-0-cwmp-full.xml", help="Target filename in data/cwmp-data-models/")
